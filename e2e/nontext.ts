 ...
 *
 * This lab is a live example of why the first half matters, and of how a check
 * can look like coverage while confirming itself. `style.css` defines
 * `--control-border`; it was applied to exactly TWO selectors,
 * `.param-row select` and `.poly-select-label select`. Every other control on
 * the page — `.ghost-button`, `.message-input`, `.theme-toggle`, `.icon-button`,
 * `.preset-chip`, `.copy-button` — drew its edge from `--line-strong` or a
 * `color-mix()`, which are SURFACE dividers. The repo also shipped
 * `e2e/border-contrast.spec.ts`, which asserted 3:1 on
 * `'.param-row select, .poly-select-label select'`: the exact four elements the
 * correct token was already on. It could not fail, and it was deleted rather
 * than repaired — this file supersedes it, over every control, against the
 * composited surface rather than a declared endpoint.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * layer (`url()`, `none`) is unmeasurable and paints nothing. Both branches
     * are load-bearing here: `.action-button` IS a
     * `linear-gradient(135deg, …)`, so its fill differs across its own width,
     * and the canvas it sits on is two `radial-gradient()` washes over a
     * `linear-gradient()`, seen through translucent panels.
     */
     ...
     *
     * A driven pass here walks the whole coefficient matrix, every `.byte-cell`
     * of every signature, target and fingerprint, and both forge cards — each
     * re-walking the same ancestors up to `<body>` through several translucent
     * panels. Nothing mutates the DOM during a pass, so the cached values cannot
     * go stale.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      for (const side of paintedSides) {
        ...
        // A drawn edge only DELINEATES if it is distinguishable from BOTH sides
        // of itself: from the surface beyond the control, and from the control's
        // own fill. Scoring it on the surround alone passes a border that is
        // invisible against the button it is drawn on — which is this lab's
        // exact shape, since `.ghost-button` is `--panel-strong` on a `--panel`
        // card 1.1:1 away and the border is doing all the work.
        //
        // This leg used to live in a second, separate oracle in `gate.ts` that
        // read `backgroundColor` only. That oracle reported a flat 1:1 for
        // `.action-button`, whose fill is a `linear-gradient` and therefore has
        // no background-COLOR at all, and it resolved every backdrop on this
        // page to WHITE, because `:root`'s background is a gradient and every
        // panel above it is translucent. Both are fabrications. Merging the rule
        // into the paint-aware core here keeps the strictness and drops the
        // false positives.
        const borderRatio = Math.min(ratio(border, surround), ratio(border, fill));
        if (borderRatio > best) {
          best = borderRatio;
          how = `border-${side} ${borderRatio.toFixed(2)}:1 (min of vs-surround and vs-fill)`;
        }
      }
      if (hasOutline) {
