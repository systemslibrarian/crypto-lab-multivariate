 ...
 *
 *  - text over a surface axe declines to resolve. This page is made of them.
 *    The document canvas is `:root`'s two `radial-gradient()` washes over a
 *    `linear-gradient()` `--surface-bg`; every `.lab-section` and `.panel-card`
 *    above it is a TRANSLUCENT `--panel` / `--panel-strong`, so that gradient
 *    shows through everywhere; `.hero-metric-card` and every `--metric-*` ink
 *    sit on a `linear-gradient()` `--metric-bg`; `.action-button` is a
 *    `linear-gradient(135deg, var(--accent), color-mix(in oklab, …))`; and the
 *    anatomy cells, the copy button, the warning banner and the forge cards are
 *    all `color-mix(in oklab, …)`. axe files that entire set under `incomplete`,
 *    which is why the violations-only gate this replaces measured almost none of
 *    the surfaces this lab actually paints.
 ...
 *    de-emphasis in several places that are NOT inactive controls, and the
 *    compositing model below is the only thing that measures them: the
 *    scoreboard and the back-to-top button drop to `0.4` for the whole length of
 *    the guided tour, `.hero-actions__quiet` sits at `0.85`, and
 *    `.anatomy-cell--oo .anatomy-cell__zero` at `0.7`.
 *
 ...
 *    boxes are `.anatomy-scroller` and the linear-system host (`overflow-x:
 *    auto` around a coefficient matrix that is n x n over GF(256), so it
 *    exceeds its panel as soon as v = 8), `.section-nav__inner` (`overflow-x:
 *    auto` at phone width) and `.result-modal__card` (`overflow-y: auto`).
 *    Content scrolled past those is not dimmed or partly drawn — it is absent
 *    from the frame, and asking what colour it sits on has no answer. Its rect
 *    is still outside every ancestor's box, so the ancestor walk would find
 ...
 *    has one SVG figure, `.trapdoor-svg`, whose four `<text>` labels sit over
 *    two stroke-only `<rect fill="none">` frames. The `fill: none` guard below
 *    is what stops those frames being composited as opaque black rectangles
 *    covering the labels — SVG's initial `fill` is black and `getComputedStyle`
 *    reports exactly that for stroke-only geometry, which invented a 3.82:1
 ...
 * one, nor is it one to axe's `color-contrast` rule. `nontext.ts` covers that
 * gap and is called from the same `scan()`.
 *
 * SECOND, and live on this page: a CSS `filter`. The compositor applies it AFTER
 * every property this walk and axe read, so no oracle can see it in either
 * direction. It was found here on `.anatomy-matrix.phase-4` and measured from
 * real screenshot pixels rather than from the stylesheet; see the note on that
 * rule in `extra.css` for what the measurement said.
 */
 ...
 *
 * There is no `within` narrowing parameter, deliberately: a parameter no call
 * site uses would be another lab's helper wearing this lab's filename. The
 * short-lived states this lab does have are driven and waited on rather than
 * sampled for — `.copy-button.is-copied`, and the four phases of the
 * quadratic-to-linear collapse, which the gate steps through by waiting on the
 * `phase-N` class the exhibit itself sets.
 */
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A driven pass here walks an n x n coefficient matrix rendered one `<span>`
     * per cell — 81 cells at v=6,o=3 and 144 at v=8,o=4 — plus one `.byte-cell`
     * per signature, target, fingerprint and forgery byte, each re-walking the
     * same ancestors up to `<body>` through several translucent panels. Without
     * the caches the pass re-reads the same computed styles and rects tens of
     * thousands of times. Nothing mutates the DOM during a pass, so the cached
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details>` of its own today; the guard stays because the gate this
      // replaces opened every one it could find from script (`d.open = true`),
      // which is precisely the habit that makes a closed disclosure look
      ...
      //
      // This lab uses the idiom heavily, which is why the guard is here and not
      // merely inherited. `style.css` defines `.sr-only` as
      // `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`, and
      // EVERY `byteGrid()` opens with a `<span class="sr-only" data-byte-hex>`
      // carrying the whole hex string — the signature, the target, the public
      // key fingerprint, every forged signature — so a driven pass meets a dozen
      // of them. The `<h1>` uses it too (`Oil <span aria-hidden>&</span><span
      // class="sr-only">and</span> Vinegar`), and `#sr-live` is an
      // `aria-live="polite"` region whose text changes on every action. Each has
      // a rect and passes `checkVisibility()` while painting exactly zero
      // pixels. It is deliberately narrow: only a ZERO-AREA clip qualifies, so a
     ...
     *
     * `.trapdoor-svg` puts every label in a `<text>`, so nothing here currently
     * trips the guard. It stays because the usual way the mistake arrives is
     * exactly how this lab builds its DOM: every render function is one large
     * `innerHTML` template literal. Interpolating a string ARRAY into one makes
     * JS join it with commas, leaving a run of "," text nodes inside whatever
     * element wraps it — and inside an SVG container those render nothing, while
     ...
     *
     * It is load-bearing here rather than defensive: this lab paints its canvas
     * ONLY on `:root`, as two `radial-gradient()` washes over a
     * `linear-gradient()` `--surface-bg`, and `<body>` declares no background at
     * all. Four elements sit outside `#app` directly on that canvas with no
     * surface of their own — `.back-to-top`, `.tour-caption`, `.replay-hint` and
     * `.scoreboard` — so without this every one of them would be measured
     * against white, in a dark theme whose canvas is #110e1a.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(rootSelector))) {
      ...
      // `.trapdoor-svg`'s four labels each carry their own `fill` attribute
      // (`--ink-strong`, `--ink-soft`, `--accent-4`) and inherit an unrelated
      // `color` from the metric card around them.
